...
# 将项目根目录添加到 Python 路径
sys.path.insert(0, '/Users/lulei/codes/ai/nanobot')
...
async def create_user_via_api(session: aiohttp.ClientSession, user_id: str) -> dict:
    """通过API创建新用户"""
    print(f"\n{'='*60}")
...
async def update_watchlist_via_api(session: aiohttp.ClientSession, user_id: str) -> dict:
    """通过API更新用户关注列表"""
    print(f"\n{'='*60}")
...
async def update_preferences_via_api(session: aiohttp.ClientSession, user_id: str, report_time: str = "14:20") -> dict:
    """通过API更新用户偏好设置"""
    print(f"\n{'='*60}")
...
async def trigger_report_immediately(user_id: str, report_type: str = "daily"):
    """直接调用调度器立即生成报告"""
    print(f"\n{'='*60}")
    ...
    try:
        # 立即生成报告
        report_id = await scheduler.generate_report_now(user_id, report_type)
        ...
        if report_id:
            ...
            print(f"\n  报告已保存到: ~/.nanobot/workspaces/user_{user_id}/reports/")
            return {"status": "completed", "user_id": user_id, "report_id": report_id}
        else:
            print(f"✗ 报告生成失败")
            return None
    finally:
...
async def setup_multi_user_schedule_with_scheduler(session: aiohttp.ClientSession) -> list:
    """使用调度器设置多个用户的定时报告任务"""
    print(f"\n{'='*60}")
    ...
    for user in users:
        ...
        # 创建用户
        user_result = await create_user_via_api(session, user['id'])
        if user_result:
            # 设置关注列表
            await update_watchlist_for_user_api(session, user['id'], user['stocks'])
            
            # 设置偏好（报告时间）
            await update_preferences_for_user_api(
                session, 
                user['id'], 
                user['report_time'], 
...
async def update_watchlist_for_user_api(session: aiohttp.ClientSession, user_id: str, stocks: list):
    """为特定用户更新关注列表"""
    url = f"{BASE_URL}/users/{user_id}/watchlist"
    data = {
        "stocks": stocks,
...
async def update_preferences_for_user_api(
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # 1.1 创建用户
        user_result = await create_user_via_api(session, test_user_id)
        if user_result:
            # 1.2 更新关注列表
            await update_watchlist_via_api(session, test_user_id)
            
            # 1.3 更新偏好设置 - 设置为 14:20
            await update_preferences_via_api(session, test_user_id, report_time="14:20")
        ...
        if results["scenario1"]:
            ...
            # 使用调度器立即生成报告
            report_result = await trigger_report_immediately(test_user_id, report_type="daily")
        ...
        users = await setup_multi_user_schedule_with_scheduler(session)
